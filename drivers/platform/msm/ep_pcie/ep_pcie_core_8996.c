...
static void pcie_core_configmhigate(struct ep_pcie_dev_t *dev)
{
	u32 mhi_ctrl_val = BIT(1) | BIT(0);
	 ...
	 * MHI are routed appropriately to the MHI registers within the PCIe
	 * core.
	 */
	ep_pcie_write_reg(dev->parf, PCIE20_PARF_MHI_BASE_ADDR_LOWER, 0x607000);
...
static void ep_pcie_core_init(struct ep_pcie_dev_t *dev)
{
	...
	ep_pcie_write_mask(dev->dm_core + TYPE0_CAP_ID_NXT_PTR_REG_1,
			0xF8000000, BIT(31) | BIT(30) | BIT(27));

	/* Set the frequency for the AUX clock to 19.2MHz */
	ep_pcie_write_mask(dev->dm_core + TYPE0_AUX_CLK_FREQ_REG,
			0x3FF, BIT(4) | BIT(2));

	/* Set the Endpoint L0s Acceptable Latency to 1us (max) */
	ep_pcie_write_mask(dev->dm_core + TYPE0_DEVICE_CAPABILITIES_REG_1,
	...
	ep_pcie_write_mask(dev->dm_core + TYPE0_LINK_CAPABILITIES_REG_1,
	...
	/* T_Power_Off */
	ep_pcie_write_mask(dev->dm_core + TYPE0_L1_SUBSTATES_REG,
			BIT(1) | BIT(0), 0);

	/* Set Device ID and Vendor ID */
	ep_pcie_write_mask(dev->dm_core + PCIE20_MISC_CONTROL_1, 0, BIT(0));
	ep_pcie_write_reg(dev->dm_core, TYPE0_DEVICE_ID_VENDOR_ID_REG_1,
			0x030217cb);
	ep_pcie_write_mask(dev->dm_core + PCIE20_MISC_CONTROL_1, BIT(0), 0);
...
int ep_pcie_core_enable_endpoint(enum ep_pcie_options opt)
{
	...
	do {
		...
		retries++;
		if (retries % 100 == 0) {
			val = readl_relaxed(dev->phy +
					PCIE_COM_PCS_READY_STATUS);
			EP_PCIE_DBG(dev,
				"PCIe V%d: PCIE_COM_PCS_READY_STATUS:0x%08x.\n",
				dev->rev, val);
		}
