		...
		struct drm_atomic_state *state)
{
	...
	if (valid_commit && edrm_kms->lk_running_flag) {
		...
		if (edrm_splash_get_lk_status(kms) != SPLASH_STATUS_NOT_START)
		{
...
static int _edrm_kms_parse_dt(struct msm_edrm_kms *edrm_kms)
{
	...
	for_each_child_of_node(parent, node) {
		...
		plane_node = of_parse_phandle(node, "qcom,assigned_plane", 0);

		/* Initialize plane */
		if (!plane_node)
			pr_err("cannot get qcom,assigned plane for %s\n",
				clabel);
		else {
			pr_err("plane node %s is valid\n", plane_node->name);
		...
		/* Initialize crtc */
		crtc = edrm_crtc_init(edrm_kms->dev, &edrm_kms->display[disp_cnt],
				priv->planes[disp_cnt]);
		if (IS_ERR(crtc)) {
			ret = PTR_ERR(crtc);
			goto fail;
		}
		...
		connector = edrm_connector_init(edrm_kms->dev,
			priv->encoders[disp_cnt], &edrm_kms->display[disp_cnt]);
		if (IS_ERR(encoder)) {
