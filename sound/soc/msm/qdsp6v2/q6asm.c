		                        ...
		                        uint32_t format, uint16_t bits_per_sample, bool ts_mode)
{
	if (q6asm_get_svc_version(APR_SVC_ASM) >= ADSP_ASM_API_VERSION_V2)
		return q6asm_open_read_v5(ac, FORMAT_LINEAR_PCM, bits_per_sample, false);
	else
                return q6asm_open_read_v4(ac, FORMAT_LINEAR_PCM, bits_per_sample, false);
}
					...
					uint16_t bits_per_sample, bool ts_mode)
{
	...
	for (i = 0; i < ASM_ACTIVE_STREAMS_ALLOWED; i++) {
		rc = q6asm_open_read_version_adaptor(ac, format, bits_per_sample, ts_mode);
		if (rc != -EALREADY)
			break;
           ...
           bool is_gapless_mode)
{
	...
	for (i = 0; i < ASM_ACTIVE_STREAMS_ALLOWED; i++) {
		pr_debug("%s: session %d \n", __func__, ac->session);
		rc = q6asm_stream_open_write_v4(ac, format, bits_per_sample, stream_id, is_gapless_mode);
		if (rc != -EALREADY)
			break;
		pr_debug("%s: session %d is occupied, try next\n", __func__, ac->session);
		q6asm_session_set_ignore(ac->session);
